...
// 方向参数状态：0=直道参数，1=弯道参数；速度本身使用参考库四档分级。
enum
{
...
extern int speed_straight_speed;          // 第0档速度：|加权偏差|<1px，单位cm/s
extern int speed_mid_fast_speed;          // 第1档速度：|加权偏差|<4px，单位cm/s
extern int speed_mid_slow_speed;          // 第2档速度：|加权偏差|<8px，单位cm/s
extern int speed_corner_speed;            // 第3档速度：|加权偏差|>=8px，单位cm/s
...
extern int speed_state;                   // 当前方向参数状态：0=直道参数，1=弯道参数
extern int speed_level;                   // 当前速度档位：0、1、2、3对应四档偏差范围
extern int speed_decision_speed;          // 四档决策和加减速限制后的速度指令，单位cm/s
extern float speed_accel_step;            // 每个图像帧允许增加的最大速度，单位cm/s
extern float speed_decel_step;            // 每个图像帧允许减少的最大速度，单位cm/s

// 使用参考库的1/4/8像素四档逻辑更新速度，每个新图像帧调用一次。
...
int16 steering_get_far_error(void);   // 读取最近一帧远点中线偏差，供无线遥测使用。
int16 steering_get_speed_error(void); // 读取四档速度决策使用的abs(加权偏差)。
