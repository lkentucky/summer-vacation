...
void Init_menu(void) {
  ...
  MenuItem* xunxian_folder_yaw= dynamic_create_menu_folder(&head, "xunxian_yaw");
  MenuItem* xunxian_folder_vision = dynamic_create_menu_folder(&head, "xunxian_vision");
  ...
  dynamic_create_menu_txt(xunxian_folder_yaw, "run_speed", &run_base_speed, int32_box);
  // 串级方向控制参数：视觉PD外环生成yaw_ref，角速度P内环跟踪yaw_ref。
  dynamic_create_menu_txt(xunxian_folder_vision, "vision_kd", &vision_yaw_kd, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "vision_ff", &vision_yaw_kff, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "yaw_max", &yaw_rate_limit_dps, int32_box);
  dynamic_create_menu_txt(xunxian_folder_yaw, "gyro_z", &imu_gyro_z_dps_filter, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "width_k", &mid_single_edge_width_scale, float_box);
#if SPEED_DECISION_ENABLE
  // 参考库四档速度：加权偏差绝对值按1、4、8像素分级。
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_0", &speed_straight_speed, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_1", &speed_mid_fast_speed, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_2", &speed_mid_slow_speed, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_3", &speed_corner_speed, int32_box);
  dynamic_create_menu_txt(xunxian_folder_yaw, "yaw_str", &speed_straight_yaw_feedback_sign, float_box);
  dynamic_create_menu_txt(xunxian_folder_yaw, "yaw_cur", &speed_corner_yaw_feedback_sign, float_box);
  dynamic_create_menu_txt(xunxian_folder_yaw, "ykp_str", &speed_straight_yaw_rate_kp, float_box);
  dynamic_create_menu_txt(xunxian_folder_yaw, "ykp_cur", &speed_corner_yaw_rate_kp, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "kp_str", &speed_straight_vision_kp, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "kp_cur", &speed_corner_vision_kp, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_state", &speed_state, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_level", &speed_level, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_cmd", &speed_decision_speed, int32_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_up", &speed_accel_step, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "spd_down", &speed_decel_step, float_box);
#else
  dynamic_create_menu_txt(xunxian_folder_vision, "vision_kp", &vision_yaw_kp, float_box);
  dynamic_create_menu_txt(xunxian_folder_vision, "yaw_kp", &yaw_rate_kp, float_box);
#endif
