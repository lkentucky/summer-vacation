...
void motor_speed_test_tick(void);       // TIM6: speed PID only, fixed equal targets.
void motor_speed_test_send_vofa(void);  // Main loop: UART1 JustFloat, never in ISR.
void motor_speed_test_uart_init(void);
void motor_speed_test_uart_rx(void);    // UART1 ISR: buffer bytes only.
void motor_speed_test_process_rx(void); // Main loop: PID <kp> <ki> <kd> + newline.
#endif
