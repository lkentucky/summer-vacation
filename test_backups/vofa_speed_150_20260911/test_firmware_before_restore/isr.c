...
// Camera diagnostics: count events only; keep display work out of interrupts.
volatile uint32 camera_vsync_irq_count = 0;
volatile uint32 camera_dma_complete_count = 0;
volatile uint32 camera_dma_error_count = 0;
...
// The temporary speed test uses neither the steering divider nor fixed PWM.
#elif MOTOR_PWM_TEST_ENABLE
...
#if MOTOR_SPEED_TEST_ENABLE
    motor_speed_test_tick(); // Fixed 150 cm/s per wheel, existing speed PID.
#elif MOTOR_PWM_TEST_ENABLE
...
void UART1_IRQHandler (void)
{
    ...
    {
#if MOTOR_SPEED_TEST_ENABLE
        motor_speed_test_uart_rx(); // Buffer only; parsing stays outside ISR.
#elif DEBUG_UART_USE_INTERRUPT                                                 // 如果开启 debug 串口中断
...
{
    uint32 camera_dma_status = DMA1->ISR;
    if(camera_dma_status & (0x00000002 << (3 * 4)))
    {
        camera_dma_complete_count++;
    }
    if(camera_dma_status & (0x00000008 << (3 * 4)))
    {
        camera_dma_error_count++;
    }
