...
#if MOTOR_SPEED_TEST_ENABLE
static volatile uint8 speed_test_running = 0;
static volatile uint32 speed_test_start_tick = 0;
static volatile float speed_test_channels[5] = {0};
#define SPEED_TEST_RX_SIZE (128U)
#define SPEED_TEST_LINE_SIZE (64U)
static volatile uint8 speed_test_rx[SPEED_TEST_RX_SIZE];
static volatile uint32 speed_test_rx_head = 0, speed_test_rx_tail = 0;
static volatile uint8 speed_test_rx_overflow = 0;
static volatile uint32 speed_test_last_rx_tick = 0;
static char speed_test_line[SPEED_TEST_LINE_SIZE];
static uint32 speed_test_line_length = 0;
static uint8 speed_test_discard_line = 0;
// CH8: 0=none, 1=applied, -1=format, -2=range, -3=overflow, -4=timeout.
static float speed_test_command_status = 0.0f;
#endif
...
float vision_yaw_kp = 1.50f;        // 最小有效P，单位(deg/s)/pixel。
float vision_yaw_kq = 0.03f;       // P随误差绝对值增加的斜率。
...
// 固定使用的IMU角速度反馈方向/比例。
float yaw_rate_feedback_sign = -0.55f;
...
{
#if MOTOR_SPEED_TEST_ENABLE
  uint32 saved_primask = __get_PRIMASK();
  __disable_irq();
  speed_test_running = 0;
#endif
...
void motor_auto_start(void)
{
  ...
  __disable_irq();
  motor_joystick_stop();
  speed_test_start_tick = g_sys_tick;
  base_speed = (int)MOTOR_SPEED_TEST_TARGET_CM_S;
  target_speedl = target_speedr = MOTOR_SPEED_TEST_TARGET_CM_S;
  speed_test_running = 1;
  __set_PRIMASK(saved_primask);
#else
...
#if MOTOR_SPEED_TEST_ENABLE
// Called after the normal encoder sample/conversion, at the unchanged 2 ms rate.
void motor_speed_test_tick(void)
{
    if (speed_test_running &&
        (uint32)(g_sys_tick - speed_test_start_tick) >=
        (MOTOR_SPEED_TEST_TIMEOUT_MS + SYS_TICK_MS - 1U) / SYS_TICK_MS)
    {
        motor_joystick_stop();
    }
    if (speed_test_running)
    ...
    {
        target_speedl = target_speedr = 0.0f;
        motorl_set_pwm(0);
        motorr_set_pwm(0);
    }
    speed_test_channels[0] = real_speedl;
    speed_test_channels[1] = real_speedr;
    speed_test_channels[2] = target_speedl; // Identical left/right target.
    speed_test_channels[3] = speed_test_running ? (float)(int)control_effortl : 0.0f;
    speed_test_channels[4] = speed_test_running ? (float)(int)control_effortr : 0.0f;
}

void motor_speed_test_send_vofa(void)
{
    static uint32 last_send_tick = 0;
    static const uint8 tail[4] = {0x00, 0x00, 0x80, 0x7f};
    float channels[9];
    uint32 now = g_sys_tick;
    uint32 saved_primask;
    uint8 i;
    if ((uint32)(now - last_send_tick) <
        (MOTOR_SPEED_TEST_SEND_MS + SYS_TICK_MS - 1U) / SYS_TICK_MS) return;
    last_send_tick = now;
    saved_primask = __get_PRIMASK();
    __disable_irq();
    for (i = 0; i < 5; i++) channels[i] = speed_test_channels[i];
    channels[5] = Kp;
    channels[6] = Ki;
    channels[7] = Kd;
    channels[8] = speed_test_command_status;
    __set_PRIMASK(saved_primask);
    // UART transmission is outside the critical section and outside TIM6.
    uart_write_buffer(DEBUG_UART_INDEX, (const uint8 *)channels, sizeof(channels));
    uart_write_buffer(DEBUG_UART_INDEX, tail, sizeof(tail));
}

void motor_speed_test_uart_init(void)
{
    speed_test_rx_head = speed_test_rx_tail = 0;
    speed_test_rx_overflow = 0;
    speed_test_line_length = 0;
    speed_test_discard_line = 0;
    speed_test_command_status = 0.0f;
    uart_rx_interrupt(DEBUG_UART_INDEX, 1);
}

void motor_speed_test_uart_rx(void)
{
    uint8 data;
    uint32 next;
    if (!uart_query_byte(DEBUG_UART_INDEX, &data)) return;
    speed_test_last_rx_tick = g_sys_tick;
    next = (speed_test_rx_head + 1U) % SPEED_TEST_RX_SIZE;
    if (next == speed_test_rx_tail)
    {
        speed_test_rx_overflow = 1;
        return;
    }
    speed_test_rx[speed_test_rx_head] = data;
    speed_test_rx_head = next;
}

static char *speed_test_skip_spaces(char *p)
{
    while (*p && isspace((unsigned char)*p)) p++;
    return p;
}

static void speed_test_apply_pid(char *line)
{
    char *p = speed_test_skip_spaces(line);
    char *end;
    float values[3];
    uint8 i;
    uint32 saved_primask;
    if (!*p) return;
    speed_test_command_status = -1.0f;
    if (toupper((unsigned char)p[0]) != 'P' ||
        toupper((unsigned char)p[1]) != 'I' ||
        toupper((unsigned char)p[2]) != 'D' ||
        !isspace((unsigned char)p[3])) return;
    p += 3;
    for (i = 0; i < 3; i++)
    {
        p = speed_test_skip_spaces(p);
        values[i] = strtof(p, &end);
        if (end == p || (*end && !isspace((unsigned char)*end))) return;
        p = end;
    }
    if (*speed_test_skip_spaces(p)) return;
    // Negated inclusive comparisons reject NaN and infinities as well.
    if (!(values[0] >= 0.0f && values[0] <= 100.0f &&
          values[1] >= 0.0f && values[1] <= 20.0f &&
          values[2] >= 0.0f && values[2] <= 20.0f))
    {
        speed_test_command_status = -2.0f;
        return;
    }
    saved_primask = __get_PRIMASK();
    __disable_irq();
    ...
    Kd = values[2];
    __set_PRIMASK(saved_primask);
    // Keep the running state, deadline and accumulated PWM unchanged.
    // Do not send ASCII acknowledgements into the JustFloat waveform stream.
    speed_test_command_status = 1.0f;
}

void motor_speed_test_process_rx(void)
{
    uint32 budget = SPEED_TEST_RX_SIZE;
    uint32 saved_primask;
    uint8 data;
    if ((speed_test_line_length || speed_test_discard_line) &&
        (uint32)(g_sys_tick - speed_test_last_rx_tick) >= (1000U / SYS_TICK_MS))
    {
        speed_test_line_length = 0;
        speed_test_discard_line = 0;
        speed_test_command_status = -4.0f;
    }
    while (budget--)
    {
        // Take one byte and handle overflow atomically against the UART ISR.
        saved_primask = __get_PRIMASK();
        __disable_irq();
        if (speed_test_rx_overflow)
        {
            speed_test_rx_tail = speed_test_rx_head;
            speed_test_rx_overflow = 0;
            speed_test_line_length = 0;
            speed_test_discard_line = 1;
            speed_test_command_status = -3.0f;
        }
        if (speed_test_rx_tail == speed_test_rx_head)
        {
            __set_PRIMASK(saved_primask);
            break;
        }
        data = speed_test_rx[speed_test_rx_tail];
        speed_test_rx_tail = (speed_test_rx_tail + 1U) % SPEED_TEST_RX_SIZE;
        __set_PRIMASK(saved_primask);
        if (data == '\r' || data == '\n')
        {
            if (!speed_test_discard_line && speed_test_line_length)
            {
                speed_test_line[speed_test_line_length] = '\0';
                speed_test_apply_pid(speed_test_line);
            }
            speed_test_line_length = 0;
            speed_test_discard_line = 0;
        }
        else if (!speed_test_discard_line)
        {
            if ((data < 32 && data != '\t') || data > 126 ||
                speed_test_line_length >= SPEED_TEST_LINE_SIZE - 1U)
            {
                speed_test_discard_line = 1;
                speed_test_command_status = -3.0f;
            }
            else speed_test_line[speed_test_line_length++] = (char)data;
        }
    }
}
#endif
